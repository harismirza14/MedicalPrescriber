...
export default function DoctorSchedule({ prescriberId }) {
  const { slots, loading, error, addSlot, deleteSlot } =
    useAvailability(prescriberId);
  ...
  return (
    <div className="w-full px-3 space-y-6">
              ...
              >
                ...
                <div className="flex flex-wrap gap-2 mt-2">
                  {daySlots.map((slot) => (
                    <span
                      key={slot.id}
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"
                    >
      ...
      <ConfirmationModal
        ...
        message={
          deletingSlot
            ? `Remove ${formatTime(deletingSlot.start_time)} – ${formatTime(deletingSlot.end_time)} on ${capitalize(deletingSlot.day_of_week)}?`
            : ""
        }
