...
export default function MedicationCard({
  ...
  if (med.status === "success") {
    // Split statusLabel into "Successfully sent" and the rest
    let successText = "";
    let restText = "";
    if (med.statusLabel) {
  ...
  if (med.status === "failed") {
    // Split statusLabel into "Failed to send" and the rest
    let failedText = "";
    let restText = "";
    if (med.statusLabel) {
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-4 mb-3">
        ...
        {med.discontinuedOn && (
          <p className="text-sm mb-1">
            ...
            <span className="text-gary-500">{med.discontinuedOn}</span>
          </p>
        )}
