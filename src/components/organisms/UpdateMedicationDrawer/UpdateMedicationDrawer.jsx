...
import { X, CirclePlus, Hospital } from 'lucide-react';
...
export default function UpdateMedicationDrawer({
  ...
  useEffect(() => {
    if (showPharmacySelector) {
      const defaultZip =
        med.pharmacy_zip || getZipFromAddress(patient?.address);
      if (defaultZip) {
  ...
  }, [showPharmacySelector, patient?.address, med.pharmacy_zip]);
                    ...
                    <>
                      ...
                      <div className="space-y-2.5 max-h-64 overflow-y-auto">
                        {filteredPharmacies.map((pharmacy) => {
                          const is24Hours = pharmacy.hours
                            ?.toLowerCase()
                            .includes("24");
                          const isSelected =
