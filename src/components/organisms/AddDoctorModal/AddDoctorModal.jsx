...
export default function AddDoctorModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setUploadedPicture(initialData?.profile_picture || null);
      setError(null);
      setSuccess(false);
    }
  }, [isOpen, initialData]);
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {success && (
            <div className="bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 p-3 rounded mb-4">
              Doctor added successfully. A temporary password has been emailed
              to them.
            </div>
          )}
          ...
          <div className="mb-4 flex justify-center">
            {isEditMode ? (
              <ProfilePicture
                currentPicture={uploadedPicture}
                name={formData.name || "Doctor"}
                onUploadSuccess={handlePictureUpload}
                size="lg"
                targetUserId={isEditMode ? initialData?.user_id : null}
                targetRoleSpecificId={
                  isEditMode ? initialData?.prescriber_id : null
                }
              />
            ) : (
              <Avatar name={formData.name || "Doctor"} size="lg" />
            )}
          </div>
